...
FrameBuffer::FrameBuffer(const uint32_t width, const uint32_t height)
  ...
  m_ColorAttachment = CreateScope<Math::Vec4[]>(m_PixelSize);
}
...
void FrameBuffer::SetColor(const uint32_t x, const uint32_t y,
  ...
  auto index = GetPixelIndex(x, y);
  if (index < m_PixelSize && index >= 0) {
    m_ColorAttachment[index] = Math::Vec4{color};
  } else {
    ASSERT(false);
  }
}

Math::Vec4 FrameBuffer::GetColor(const uint32_t x, const uint32_t y) const {
  auto index = GetPixelIndex(x, y);
  if (index < m_PixelSize && index >= 0) {
...
void FrameBuffer::Clear(const Math::Vec4 &color) {
